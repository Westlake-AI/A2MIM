...
def load_checkpoint(model_name, checkpoint_path, model):
    """ load pretrained openmixup models """
    ...
    for key, value in sd.items():
        ...
        replace = False
        # convert ema
        if ('ema_' in new_key) and ('deit' in model_name or 'vit' in model_name or 'swin' in model_name):
            replace = True
            new_key = new_key.replace("ema_", "")
            new_key = new_key.replace("_", ".")
            ema_key_list = [
                'cls_token', 'pos_embed', 'patch_embed',
                'absolute_pos_embed', 'relative_position_bias_table', 'w_msa',]
            for ori_key in ema_key_list:
                dot_key = ori_key.replace("_", ".")
                if dot_key in new_key:
                    new_key = new_key.replace(dot_key, ori_key)

        # remove backbone keys
        for prefix_k in ['encoder_q', 'encoder', 'backbone', 'timm_model',]:
            if new_key.startswith(prefix_k):
                new_key = new_key[len(prefix_k) + 1: ]
        # remove head keys
        if "resnet" not in model_name:
            if "mlpmixer" not in model_name:
                for head_k in ['head.layers.', 'fc_cls.', 'fc.',]:
                    start_idx = new_key.find(head_k)
                    if start_idx != -1:
                        new_key = new_key[:start_idx] + new_key[start_idx + len(head_k): ]
        else:
            for head_k in ['head.',]:
                start_idx = new_key.find(head_k)
                if start_idx != -1:
        ...
        if 'deit' in model_name or 'vit' in model_name:
            ...
            if new_key.find('gamma_') != -1:
                new_key = new_key.replace('gamma_', 'ls')
            ...
            if new_key.find('head.fc.') != -1:
                new_key = new_key.replace('head.fc.', 'head.')
        
        elif 'swin' in model_name:
            for _k in ['w_msa',]:
                start_idx = new_key.find(_k)
                if start_idx != -1:
                    new_key = new_key[:start_idx] + new_key[start_idx + len(_k)+1: ]
            if new_key.find('projection') != -1:
                new_key = new_key.replace('projection', 'proj')
            if new_key.find('ffn.layers.0.0.') != -1:
                new_key = new_key.replace('ffn.layers.0.0.', 'mlp.fc1.')
            ...
            if new_key.find('stages.') != -1:
                new_key = new_key.replace('stages.', 'layers.')
            if new_key == 'norm3.weight':
                new_key = 'norm.weight'
            if new_key == 'norm3.bias':
                new_key = 'norm.bias'
        
        # replace
        if new_key in load_dict.keys():
            if not replace:
                # print(f"EMA: remove the original key {new_key}")
                continue
        
        load_dict[new_key] = value
        print(f"keep key {key} -> {new_key}")
    
    load_state_dict(model, load_dict, strict=False)
    return model
...
def get_model(args, pretrained=True):
    ...
    if args.model_name in model_names:
        ...
        model = timm.create_model(args.model_name, pretrained=pretrained)
        mean = (0.485, 0.456, 0.406)
        std = (0.229, 0.224, 0.225)
    elif 'resnet_drop' in args.model_name:
        model = vit_models.drop_resnet50(pretrained=pretrained)
        mean = (0.485, 0.456, 0.406)
        std = (0.229, 0.224, 0.225)
    ...
    if pretrained_path is not None:
        model = load_checkpoint(args.model_name, pretrained_path, model)
...
def parse_args():
    ...
    parser.add_argument('--test_list', type=str, default=None, help='Meta file for custom dataset')
    ...
    parser.add_argument('--num_classes', type=int, default=None, help='Reset class number')
    parser.add_argument('--pretrained_path', type=str, default=None, help='Path to custom pretrained model')
